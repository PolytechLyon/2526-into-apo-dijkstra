...
Nous voulons pouvoir calculer le trajet optimal, c'est-à-dire avec la durée totale la plus courte,
à partir d'une ville donnée et vers toutes les autres villes accèssibles par train depuis la ville de départ.
...
* Une liste des connexions entre les villes. Les connexions sont séparés par des retours à la ligne.
...
Proposer des structures de données adaptées permettant une représentation d'un graphe orienté
...
Pour construire ce graphe, vous pouvez écrire un code qui crée, et remplit votre structure de donnée
d'une manière statique (codé en dur) en suivant les informations dans le fichier `input.txt`.
...
pour atteindre chaque autre destination, si celle-là est accessible, à partir d'une ville donnée.
...
Une variante de l'algorithme décrit ci-dessus peut être utilisé pour determiner le chemin le plus court,
...
avec leurs coûts respectifs, en partant d'une ville donnée.
